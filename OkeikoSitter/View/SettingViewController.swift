...
final class SettingViewController: UIViewController {
    ...
    /// 選択した連続記録日数
    private var selectedStreakGoalDays: Int?
    /// デリゲート
    ...
    /// 目標連続記録日数の見出し
    @IBOutlet private weak var streakGoalDaysLabel: UILabel!
    /// 選択した目標連続記録日数を表示するラベル
    @IBOutlet private weak var streakGoalDaysValueLabel: UILabel!
    /// 目標連続記録日数選択ボタン
    @IBOutlet private weak var streakGoalDaysMenuButton: PressableButton!
    ...
    override func viewDidLoad() {
        ...
        configureStreakGoalDaysMenuButton()
    ...
    private func configureUI() {
        if let user = UserSession.shared.currentUser {
            ...
            streakGoalDaysLabel.text = "連続記録日数"

            if user.streakGoalDays > 0 {
                streakGoalDaysValueLabel.text = "\(user.streakGoalDays)日"
                selectedStreakGoalDays = user.streakGoalDays
            } else {
                streakGoalDaysValueLabel.text = "日数を選択してください"
                selectedStreakGoalDays = nil
            }
    ...
    private func configureStreakGoalDaysMenuButton() {
        let streakGoalDaysMenu = UIMenu(title: "", children: [
            ...
            UIAction(title: "14日") { _ in
                self.selectedStreakGoalDays = 14
                self.streakGoalDaysValueLabel.text = "14日"
            },
            UIAction(title: "30日") { _ in
                self.selectedStreakGoalDays = 30
                self.streakGoalDaysValueLabel.text = "30日"
            }
        ])
        streakGoalDaysMenuButton.menu = streakGoalDaysMenu
        streakGoalDaysMenuButton.showsMenuAsPrimaryAction = true
    }
    ...
    private func validateSaveData(completion: ((Bool) -> Void)? = nil) {
        ...
        guard let challengeTask = challengeTaskTextView.text, !challengeTask.isEmpty,
              ...
              let streakGoalDays = selectedStreakGoalDays else {
    ...
    private func updateUserSession(with data: [String: Any]) {
        ...
        if let streakGoalDays = data["streak_goal_days"] as? Int {
            user.streakGoalDays = streakGoalDays
        }
        if let dailyPointTotals = data["daily_point_totals"] as? [String: Int] {
            user.dailyPointTotals = dailyPointTotals
        }
        if let selectedDates = data["selected_dates"] as? [TimeInterval] {
